...
const pankaj = new Person('pankaj');
const kiran = new Person('kiran');

const personsHolder = [aditya, pankaj, kiran];

personsHolder.forEach(person => {
    person.on('speak', function(message){
        console.log(person.name + ' said ' + message);
    });
});

aditya.emit('speak', 'How is it going gang?');
pankaj.emit('speak', 'nothing.');
kiran.emit('speak', 'I am coimg bruh.');